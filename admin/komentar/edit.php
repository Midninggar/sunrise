...
<?php
    ...
    $query = mysqli_query($kon, "SELECT * FROM komentar k inner join produk a on a.id_produk=k.id_produk where id_komentar=$id_komentar");
    $data = mysqli_fetch_array($query); 

    $judul_produk=$data['judul_produk'];
    $nama=$data['nama'];
    $email=$data['email'];
...
?>
    ...
    <div class="form-group">
        ...
        <div class="form-group">
            <label>Produk:</label>
            <input name="nama" value="<?php echo $judul_produk; ?>" type="text" class="form-control" disabled>
        </div>
        <div class="form-group">
            <label>Nama:</label>
...
<?php
    if (isset($_POST['simpan_edit'])) {
        ...
        $id_artikel=input($_POST["id_produk"]);
